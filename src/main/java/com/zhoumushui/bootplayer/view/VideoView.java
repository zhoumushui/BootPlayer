...
public class VideoView extends SurfaceView implements MediaPlayerControl {
	...
	private OnErrorListener mErrorListener = new OnErrorListener() {
		public boolean onError(MediaPlayer mp, int framework_err, int impl_err) {
			...
			if (mOnErrorListener != null) {
				if (mOnErrorListener.onError(mediaPlayer, framework_err,
						impl_err)) {
					return true;
				}
			}
	...
	SurfaceHolder.Callback mSHCallback = new SurfaceHolder.Callback() {
		public void surfaceChanged(SurfaceHolder holder, int format, int w,
			...
			mSurfaceWidth = w;
			mSurfaceHeight = h;
			if (mediaPlayer != null && mIsPrepared && mVideoWidth == w
					&& mVideoHeight == h) {
				if (mSeekWhenPrepared != 0) {
					mediaPlayer.seekTo(mSeekWhenPrepared);
					mSeekWhenPrepared = 0;
	...
	public boolean onTouchEvent(MotionEvent ev) {
		if (mIsPrepared && mediaPlayer != null && mMediaController != null) {
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (mIsPrepared && keyCode != KeyEvent.KEYCODE_BACK
				...
				&& keyCode != KeyEvent.KEYCODE_ENDCALL && mediaPlayer != null
